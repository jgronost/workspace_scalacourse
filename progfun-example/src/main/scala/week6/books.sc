...
//		b.authors withFilter( a => a.startsWith("Bird"))  map (y => y.title))
	...
	yield b. title                            //> res2: scala.collection.immutable.Set[String] = Set(Programming in Scala, St
...
for {
	b1 <- books
	b2 <- books
	//if b1 != b2 						//gives duplicates if books are list
	if b1.title < b2.title		//to remove duplicates
	a1 <- b1.authors
	a2 <- b2.authors
	if a1 == a2
